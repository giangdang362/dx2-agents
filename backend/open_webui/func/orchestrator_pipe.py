...
class Pipe:
    ...
    # ------------------------------------------------------------------
    # LLM helpers — delegates to open_webui.utils.llm_client
    # ------------------------------------------------------------------

    async def _call_llm(self, messages: list[dict], model: str | None = None) -> dict:
        """Non-streaming chat completion. Returns OpenAI-shaped dict."""
    ...
    async def _stream_llm(self, messages: list[dict], model: str | None = None):
        ...
        if self.valves.LLM_PROVIDER == "gemini":
            async def _gemini_lines():
                async for token in llm_client.gemini_stream(messages, model=effective_model):
                    # Wrap tokens as SSE data lines for consistency with downstream consumers
                    chunk = {
            ...
            async def _ollama_lines():
                async for line in llm_client.ollama_stream_sse(
                    messages, model=effective_model, base_url=self.valves.OLLAMA_BASE_URL
                ):
                    yield line
    ...
    async def _discover_agents(self) -> dict[str, dict]:
        ...
        # 2) Workspace models from DB (only active agents, not raw base models)
        try:
            from open_webui.models.models import Models

            for m in Models.get_models() or []:
                if m.id in ("orchestrator", "orchestrator_pipe") or m.id in agents:
                    continue
        ...
        # 3) Pipe functions (registered in Functions table, not Models table)
        #    Manifold pipes register as "{fn.id}.{sub_id}" in app.state.MODELS,
        #    so we need to resolve sub-pipe IDs to match what Open WebUI expects.
        try:
            ...
            for fn in Functions.get_functions_by_type("pipe", active_only=True) or []:
                if fn.id in ("orchestrator", "orchestrator_pipe") or fn.id in agents:
                    continue
                description = (fn.meta.description if fn.meta else None) or ""

                # Try to resolve manifold sub-pipes
                try:
                    module, _, _ = load_function_module_by_id(fn.id)
                    if hasattr(module, "pipes") and callable(module.pipes):
                        if inspect.iscoroutinefunction(module.pipes):
                            sub_pipes = await module.pipes()
                        else:
                            sub_pipes = module.pipes()
                        for sp in sub_pipes or []:
                            sub_id = f'{fn.id}.{sp["id"]}'
                            if sub_id in agents or "orchestrator" in sub_id:
                                continue
                            agents[sub_id] = {
                                "name": sp.get("name", sub_id),
                                "description": description,
                                "url": None,
                                "profile_image_url": "",
                            }
                        continue  # skip adding the base fn.id
                except Exception:
                    pass  # fall through to add as simple pipe
            ...
            print(
                f"[orchestrator-pipe] Failed to load pipe functions: {e!r}",
                flush=True,
            )
        ...
        routing_messages = [
            {"role": "system", "content": self.valves.ROUTING_SYSTEM_PROMPT},
            {
                ...
                "content": (
                    ...
                    'Each object must have "id" (agent ID), "action" (a short present-participle description '
                    "of what this agent will do for this specific request), and "
                    '"keywords" (list of keywords/phrases from the user message that led to this match).\n'
                    'Example: [{"id": "it-agent", "action": "Resetting user password", '
                    '"keywords": ["password", "reset", "can\'t log in"]}]\n'
                    "IMPORTANT: For greetings, meta-questions (e.g. 'what can you do?', "
                    "'who are you?', 'help'), or any message that is NOT a specific task "
                    "for a specialist agent, you MUST respond with exactly: NONE\n"
                    "Only route to an agent when the user has a clear, actionable request "
                    "that matches that agent's description."
                ),
            },
        ]
            ...
            if isinstance(parsed, list):
                ...
                for item in parsed:
                    if isinstance(item, dict) and item.get("id") in agents_subset:
                        ...
                        results.append({"id": item, "action": "Handling request", "keywords": []})
                return results
        except (json.JSONDecodeError, TypeError):
            pass

        if cleaned in agents_subset:
            return [{"id": cleaned, "action": "Handling request", "keywords": []}]
    ...
    async def pipe(
        ...
        async def emit(step: str, **kwargs):
            ...
            try:
                ...
                await orch_publish(event_payload)
            except Exception as e:
                print(f"[orchestrator-pipe] broadcast error: {e!r}", flush=True)
            # 2) Also emit via Socket.IO (chat status history)
        ...
        keywords_map = {r["id"]: r.get("keywords", []) for r in routed_agents}
        selected_ids = list(action_map.keys())

        # --- Emit: specialist match with keyword reasoning ---
        if selected_ids:
            match_details = []
            for agent_id in selected_ids:
                agent = accessible_agents.get(agent_id, {})
                kw = keywords_map.get(agent_id, [])
                match_details.append({
                    "agent_id": agent_id,
                    "agent_name": agent.get("name", agent_id),
                    "action": action_map.get(agent_id, ""),
                    "keywords": kw,
                })
            kw_summary = "; ".join(
                f"{d['agent_name']}: {', '.join(d['keywords']) if d['keywords'] else d['action']}"
                for d in match_details
            )
            await emit(
                "specialist_match",
                message=f"Matched by keywords: {kw_summary}",
