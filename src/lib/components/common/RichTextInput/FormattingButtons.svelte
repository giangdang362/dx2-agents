...
<div
	class="flex gap-0.5 p-0.5 rounded-xl shadow-lg bg-white text-gray-800 dark:text-white dark:bg-slate-800 min-w-fit border border-gray-100 dark:border-gray-800"
>
	<Tooltip placement="top" content={$i18n.t('H1')}>
		<button
			...
			class="{editor?.isActive('heading', { level: 1 })
				...
				: ''} hover:bg-gray-50 dark:hover:bg-slate-700 rounded-lg p-1.5 transition-all"
			type="button"
		>
			<H1 />
		</button>
	...
	{#if editor?.isActive('bulletList') || editor?.isActive('orderedList') || editor?.isActive('taskList')}
		<Tooltip placement="top" content={$i18n.t('Lift List')}>
			<button
				...
				class="hover:bg-gray-50 dark:hover:bg-slate-700 rounded-lg p-1.5 transition-all"
				type="button"
			>
				<ArrowLeftTag />
			</button>
