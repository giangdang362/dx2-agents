<script lang="ts">
	...
	import RoomList from '$lib/components/chat/Demo/MeetingRoom/RoomList.svelte';
	...
	export let onApprove: any = null;
	export let onReject: any = null;
	export let onAddNote: any = null;
	export let onOrderCatering: any = null;
	export let onApproveCatering: any = null;
	export let onRejectCatering: any = null;
	export let onSendCalendar: any = null;
	export let onRoomSelect: any = null;
	export let onUpdateDateTime: any = null;
	...
	function handleConfirmStep2() {
		if (onConfirm) onConfirm(activeBooking.id, invitees);
	...
	// Sync local form state from booking prop — runs on mount AND whenever
	// booking changes (e.g. after the DB-fetch overwrites the draft).
	function syncFromBooking(b) {
		if (!b) return;
		if (b.catering?.items?.length) {
			selectedCatering = b.catering.items;
			cateringTotal = b.catering.total || 0;
			noCatering = false;
		} else {
			noCatering = true;
			selectedCatering = [];
...
<style>
	...
	:global(.dark) .location-label {
		color: #475569;
	}

	:global(.dark) .location-value {
		color: #f1f5f9;
	}

	:global(.dark) .location-divider {
		background: #334155;
	}
	...
	:global(.dark) .no-catering-badge {
		color: #94a3b8;
		background: #1e293b;
		border-color: #334155;
	}

	:global(.dark) .catering-saved-badge {
		color: #60a5fa;
		background: rgba(59, 130, 246, 0.15);
		border-color: rgba(59, 130, 246, 0.4);
	}

	:global(.dark) .no-catering-toggle {
		background: #0f172a;
		border-color: #475569;
		color: #94a3b8;
	}

	:global(.dark) .no-catering-toggle:hover {
		background: #1e293b;
		border-color: #94a3b8;
	}

	:global(.dark) .no-catering-toggle.selected {
		background: linear-gradient(135deg, rgba(202, 138, 4, 0.2) 0%, rgba(202, 138, 4, 0.1) 100%);
		border-color: #ca8a04;
		color: #fde68a;
	}

	:global(.dark) .no-catering-toggle.selected .check-icon {
		color: #fbbf24;
	}

	:global(.dark) .catering-btn {
		background: #0f172a;
		border-color: #334155;
	}

	:global(.dark) .catering-btn:hover {
		background: #1e293b;
		border-color: #475569;
	}

	:global(.dark) .catering-btn.selected {
	...
	:global(.dark) .invitees-input {
		background: #0f172a;
		border-color: #334155;
		color: #f1f5f9;
	}

	:global(.dark) .invitees-input::placeholder {
		color: #475569;
	}

	:global(.dark) .invitees-input:focus {
		border-color: #3b82f6;
		background: #1e293b;
		box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.15);
	}

	:global(.dark) .catering-footer {
	...
	:global(.dark) .note-input::placeholder {
		color: #475569;
	}
	...
	:global(.dark) .workflow-step:not(:last-child)::after {
		background: #334155;
	}

	:global(.dark) .workflow-step.completed::after {
		background: #065f46;
	}

	:global(.dark) .workflow-step.active::after {
		background: linear-gradient(to bottom, #065f46 50%, #334155 50%);
	}

	:global(.dark) .step-marker {
		background: #334155;
		color: #94a3b8;
	}

	:global(.dark) .workflow-step.completed .step-marker {
		background: #065f46;
		color: #6ee7b7;
	}

	:global(.dark) .workflow-step.active .step-marker {
		background: #78350f;
		color: #fcd34d;
	}
	...
	:global(.dark) .step-assignee {
		color: #94a3b8;
	}

	:global(.dark) .action-btn.cancel,
	:global(.dark) .action-btn.reject {
		background: #1e293b;
		color: #94a3b8;
		border-color: #334155;
	}

	:global(.dark) .action-btn.cancel:hover,
	:global(.dark) .action-btn.reject:hover {
		background: #0f172a;
	}

	:global(.dark) .email-box {
	...
	:global(.dark) .sim-processing {
		color: #818cf8;
	}

	:global(.dark) .sim-spinner {
		border-color: #312e81;
