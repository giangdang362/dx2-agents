...
>
	...
	<DropdownMenu.Content
		strategy="fixed"
		class="w-full max-w-[210px] text-sm rounded-2xl p-1 z-[9999999] bg-white dark:bg-slate-800 dark:text-white shadow-lg border border-gray-100  dark:border-gray-800"
		sideOffset={-2}
		side="bottom"
		align="end"
	...
	>
		{#if model?.preset || model?.info?.base_model_id ? model?.info?.user_id === $user?.id : $user?.role === 'admin'}
			<DropdownMenu.Item
				type="button"
				class="select-none flex rounded-xl py-1.5 px-3 w-full hover:bg-gray-50 dark:hover:bg-slate-700 transition items-center gap-2"
				on:click={(e) => {
					e.stopPropagation();
					e.preventDefault();
