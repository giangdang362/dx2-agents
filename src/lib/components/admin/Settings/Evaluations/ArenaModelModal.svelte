<script>
	...
	let profileImageUrl = `${WEBUI_BASE_URL}/cmc-favicon.png`;
	let description = '';
	...
	const submitHandler = () => {
		...
		name = '';
		id = '';
		profileImageUrl = `${WEBUI_BASE_URL}/cmc-favicon.png`;
		description = '';
		modelIds = [];
		selectedModelId = '';
	};
			...
			<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
				...
				>
					...
					<div class="flex justify-end pt-3 text-sm font-medium gap-1.5">
						{#if edit}
							<button
								class="px-3.5 py-1.5 text-sm font-medium dark:bg-black dark:hover:bg-slate-900 dark:text-white bg-white text-black hover:bg-gray-100 transition rounded-full flex flex-row space-x-1 items-center"
								type="button"
								on:click={() => {
						...
						<button
							class="px-3.5 py-1.5 text-sm font-medium bg-black hover:bg-gray-950 text-white dark:bg-white dark:text-black dark:hover:bg-slate-100 transition rounded-full flex flex-row space-x-1 items-center {loading
								? ' cursor-not-allowed'
								: ''}"
							type="submit"
							disabled={loading}
