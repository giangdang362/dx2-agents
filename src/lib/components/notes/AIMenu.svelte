...
<DropdownMenu.Root bind:open={show} onOpenChange={onChange}>
	...
	<slot name="content">
		<DropdownMenu.Content
			class="w-full {className} text-sm rounded-xl p-1 z-50 bg-white dark:bg-slate-800 dark:text-white shadow-lg font-primary"
			sideOffset={8}
			side="bottom"
			align="end"
		...
		>
			<button
				class="flex rounded-md py-1.5 px-3 w-full hover:bg-gray-50 dark:hover:bg-slate-700 transition"
				on:click={async () => {
					onEdit();
					show = false;
				}}
